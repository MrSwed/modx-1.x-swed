	...
	text_places - Любые заданные разработчиком места расположения, Соостветственно вызов сниппета 
		...
		sel_inheritance - Настройки наследования             
		  не задано -  используются значения родителя, т.е. заданное значение будет отображено только для текухего документа, дочерние покажут значения родителя
		      0     -  "Нет". Отменить наследование всего блока
...
// todo: избавиться от двойного вызова getInheritField
if (empty($value) and "0"!=$modx->evalSnippets("[[getInheritField? &id=`{$docData["parent"]}` &field=`$tv` &runSnippet=`ddGetMultipleField? &string='%s'  &filter='1::$name||2::0||2::1||2::2".((int)$docData["isfolder"] ? "||2::3||2::4" : "||2::5")."' &columns='2'`]]")) {
	
	$value = $modx->evalSnippets("[[getInheritField? &id=`{$docData["parent"]}` &field=`$tv` &runSnippet=`ddGetMultipleField? &string='%s'  &filter='1::$name||2::1||2::2".((int)$docData["isfolder"] ? "||2::3||2::4" : "||2::5")."' &columns='0'`]]");
}
